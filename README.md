...
このシステムは複数のナレッジマネジメント手法を組み合わせています。

### 参照メソッド

| メソッド | 提唱者 | 本システムでの適用 |
|---------|--------|-------------------|
| **Building a Second Brain (BASB)** | Tiago Forte | 「外部脳」の概念、CODE framework |
| **Zettelkasten** | Niklas Luhmann | 1ノート1アイデア、リンクで繋ぐ |
| **Linking Your Thinking (LYT)** | Nick Milo | MOC（Map of Content）による索引 |
| **GTD (Getting Things Done)** | David Allen | inbox→処理→整理のフロー |

### フレームワーク対応

```
本システム          BASB (CODE)        Zettelkasten
─────────────────────────────────────────────────
Collect (inbox/)  → Capture          → 一時メモ
Distill (notes/)  → Organize+Distill → 永久保存箱
Connect (maps/)   → (なし)           → 索引ノート
Use (projects/)   → Express          → プロジェクト適用
```
...
| メソッド | 採用した考え方 |
|---------|---------------|
| **BASB** | 「使うために貯める」思想。情報は活用してこそ価値がある |
| **Zettelkasten** | 原子的ノート＋双方向リンク。知識の発見と再利用を促進 |
| **LYT** | MOCで俯瞰。トップダウンとボトムアップの両方でナビゲート |
| **GTD** | inboxを空にする習慣。未処理を可視化し、処理を促す |

### 参考書籍・リソース

- 📕 [Building a Second Brain](https://www.buildingasecondbrain.com/) - Tiago Forte
- 📗 [How to Take Smart Notes](https://www.soenkeahrens.de/en/takesmartnotes) - Sönke Ahrens (Zettelkasten解説)
